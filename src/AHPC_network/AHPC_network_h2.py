...
class PyAHPCnetworkModel(AbstractSubProcessModel):
    ...
    def __init__(self, proc):
        ...
        self.leaky1.a_in.connect_from(self.linear1.a_out)

        self.recurrent_block = RecurrentBlock(in_dense=proc.linear2_w.init,
                                                f_du=proc.leaky2_betas.init,
                                                f_vth=proc.leaky2_vth.init,
                                                r_in_weight=proc.recurrent_in_w.init,
                                                b_betas=proc.reccurrent_leaky_betas.init,
                                                r_out_weight=proc.recurrent_out_w.init,
                                                log_config= proc.log_config,
                                                name="recurrent_block"
                                            )
        self.recurrent_block.s_in.connect_from(self.leaky1.s_out)

        self.linear3 = Dense(weights=proc.linear3_w.init, num_message_bits=0)
        self.linear3.s_in.connect_from(self.recurrent_block.s_out)
        ...
        if proc.debug:
            ...
            proc.out_ports.r_current.connect_from(self.backward.c_out)
               
        proc.leaky1_v.alias(self.leaky1.v)
        proc.leaky1_u.alias(self.leaky1.u)

        proc.leaky2_v.alias(self.recurrent_block.f_v)
        proc.leaky2_u.alias(self.recurrent_block.f_u)

        proc.ahpc_v.alias(self.recurrent_block.b_v)
        proc.ahpc_u.alias(self.recurrent_block.b_u)

        proc.leaky3_v.alias(self.leaky3.v)
        proc.leaky3_u.alias(self.leaky3.u)

        proc.linear1_a_buffer.alias(self.linear1.a_buff)
        proc.linear2_a_buffer.alias(self.recurrent_block.in_buff)
        proc.linear3_a_buffer.alias(self.linear3.a_buff)

        proc.recurrent_in_a_buffer.alias(self.recurrent_block.r_in_buff)
        proc.recurrent_out_a_buffer.alias(self.recurrent_block.r_out_buff)



class RecurrentBlock(AbstractProcess):

    def __init__(self, in_dense, f_du, f_vth, r_in_weight, b_betas, r_out_weight, **params):
        
        super().__init__( **params)
        
        self.s_in  = InPort(shape = (in_dense.shape[1],))
        self.s_out  = OutPort(shape = (in_dense.shape[0],))
        
        self.in_dense = Var(shape=in_dense.shape, init=in_dense)
        self.f_du = Var(shape=f_du.shape, init=f_du)
        self.f_vth = Var(shape=f_vth.shape, init=f_vth)

        self.f_u = Var(shape=(r_in_weight.shape[0],), init=0)
        self.f_v = Var(shape=(r_in_weight.shape[0],), init=0)

        self.r_in_weight = Var(shape=r_in_weight.shape, init=r_in_weight)
        self.b_betas = Var(shape=b_betas.shape, init=b_betas)
        self.r_out_weight = Var(shape=r_out_weight.shape, init=r_out_weight)

        self.in_buff = Var(shape=(r_in_weight.shape[0],), init=0)
        self.r_in_buff = Var(shape=(r_in_weight.shape[0],), init=0)
        self.r_out_buff = Var(shape=(r_out_weight.shape[0],), init=0)
        self.b_u = Var(shape=(r_in_weight.shape[0],), init=0)
        self.b_v = Var(shape=(r_in_weight.shape[0],), init=0)
        log_config = params.pop('log_config', 0)
        self.log_config = log_config 
        
    def reset_hidden_state(self):

        self.b_v.set(np.zeros(self.v.shape))
        self.b_u.set(np.zeros(self.u.shape))
    ...
    def print_state_variables(self):
        ...
        print(f"b_betas: {self.b_betas.get()}")
        print("")

@implements(proc=RecurrentBlock, protocol=LoihiProtocol)
@requires(CPU)
class PyRecurrentBlock(AbstractSubProcessModel):

    def __init__(self, proc):
        
        self.dense_in = Dense(weights=proc.in_dense.init)
        proc.s_in.connect(self.dense_in.s_in)

        self.leaky = LIF(shape=(proc.in_dense.shape[0],),
                            u = 0,
                            v = 0,
                            dv = 1,
                            du = proc.f_du.init,
        ...
        self.leaky.a_in.connect_from(self.dense_in.a_out)
        self.leaky.s_out.connect(proc.s_out)

        self.linear_in = Dense(weights=proc.r_in_weight.init)
        self.linear_in.s_in.connect_from(self.leaky.s_out)

        self.ahpc = AHPCompartment(shape=(proc.r_in_weight.shape[0],),
                            u = 0,
                            v = 0,
                            dv = 1,
                            du = proc.b_betas.init,
                            log_config= proc.log_config,
                            name= "AHPC"
                        )
        self.linear_in.a_out.connect(self.ahpc.a_in)

        self.linear_out = Dense(weights=proc.r_out_weight.init, num_message_bits=32)
        self.linear_out.s_in.connect_from(self.ahpc.c_out)
        self.linear_out.a_out.connect(self.leaky.a_in)
        
        proc.f_v.alias(self.leaky.v)
        proc.f_u.alias(self.leaky.u)
        proc.in_buff.alias(self.dense_in.a_buff)
        proc.r_in_buff.alias(self.linear_in.a_buff)
        proc.r_out_buff.alias(self.linear_out.a_buff)
        proc.b_u.alias(self.ahpc.u)
        proc.b_v.alias(self.ahpc.v)

...
class PyAHPCnetworkModel(AbstractSubProcessModel):
    ...
    def __init__(self, proc):
        ...
        self.linear2 = Dense(weights=proc.linear2_w.init, num_message_bits=0)
        ...
        # self.backward.c_out.connect(self.leaky2.a_in)
        
        

        
        self.recurrent_in = Dense(weights=proc.recurrent_in_w.init, num_message_bits=0)
        self.recurrent_in.s_in.connect_from(self.leaky2.s_out)

        self.ahpc = AHPCompartment(shape=(proc.linear2_w.shape[0],),
                            u = 0,
                            v = 0,
                            dv = 1,
                            du = proc.reccurrent_leaky_betas.init,
                            log_config= proc.log_config,
                            name= "ahpc"
                        )
        self.ahpc.a_in.connect_from(self.recurrent_in.a_out)

        self.recurrent_out = Dense(weights=-proc.recurrent_out_w.init, num_message_bits=32, )
        self.recurrent_out.s_in.connect_from(self.ahpc.c_out)
        ...
        print(f"recurrent dv:{self.ahpc.dv.get()}")
        print(f"recurrent du:{self.ahpc.du.get()}")
        ...
        if proc.debug:
            ...
            proc.out_ports.r_current.connect_from(self.ahpc.c_out)
        ...
        proc.ahpc_v.alias(self.ahpc.v)
        proc.ahpc_u.alias(self.ahpc.u)
        ...
        proc.recurrent_in_a_buffer.alias(self.recurrent_in.a_buff)
        proc.recurrent_out_a_buffer.alias(self.recurrent_out.a_buff)
...
class BackwardBranch(AbstractProcess):
    ...
    def __init__(self, in_weight, betas, out_weight, **params):
        ...
        self.c_out  = OutPort(shape = (out_weight.shape[0],))
        
        self.in_weight = Var(shape=in_weight.shape, init=in_weight)
        self.betas = Var(shape=betas.shape, init=(1-betas))
...
class PyBackwardBranch(AbstractSubProcessModel):
    ...
    def __init__(self, proc):
        ...
        self.linear_in = Dense(weights=proc.in_weight.init, num_message_bits=32)
        proc.a_in.connect(self.linear_in.s_in)
        ...
        self.linear_out.s_in.connect(self.leaky.c_out)
        self.linear_out.a_out.connect(proc.c_out)

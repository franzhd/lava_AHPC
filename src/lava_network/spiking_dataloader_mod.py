...
class AbstractPySpikeDataloader(PyLoihiProcessModel):
...
class PySpikeDataloader(AbstractPySpikeDataloader):
    def __init__(self, proc_params):
        super().__init__(proc_params=proc_params)
        self.curr_sample_id = 0
        self.curr_sample_time_step = 0
...
@requires(CPU)
@tag("floating_pt", "fixed_pt")
class PySpikeRewardDataloader(AbstractPySpikeDataloader):
